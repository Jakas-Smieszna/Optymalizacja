...
// Pliki z wynikami typu csv znajdują się w folderze out, przykładowo dla
// debugowania x64: Optymalizacja\out\build\x64-debug\nazwa_pliku.csv Wykonanie
// zadania: MF, JG, MG, AG
...
void lab0() {
  ...
  a(0) = -1;
  a(1) = 2;
  opt = MC(ff0T, 2, lb, ub, epsilon, Nmax,
           a);                 // wywo�anie procedury optymalizacji
  ...
  opt = MC(ff0R, 1, lb, ub, epsilon, Nmax,
           teta_opt);          // wywo�anie procedury optymalizacji
  cout << opt << endl << endl; // wypisanie wyniku
  solution::clear_calls();     // wyzerowanie licznik�w

  // Zapis symulacji do pliku csv
  matrix Y0 = matrix(2, 1), // Y0 zawiera warunki pocz�tkowe
      MT = matrix(2, new double[2]{m2d(opt.x),
                                   0.5}); // MT zawiera moment si�y dzia�aj�cy
                                          // na wahad�o oraz czas dzia�ania
  matrix *Y = solve_ode(df0, 0, 0.1, 10, Y0, NAN,
                        MT);           // rozwi�zujemy r�wnanie r�niczkowe
...
void lab1() {
  ...
  while (kont == '1') {
    for (int i = 0; i < 1; i++) { // JG:mozna wybrac liczbe powtorzen
      ...
      cout << "\nKrok d = " << krok_d
           << "\tWspolczynnik ekspansji alfa = " << alfa << ".\n";
      cout << "Punkt startowy = " << ps(0) << "\tUzyskany przedzial = ["
           << obszar[0] << ", " << obszar[1]
           << "].\n"; // JG:pozwala przywrocic liczbe po ekspansji, gdy zostanie
                      // wycyszczona.
      cout << "EKSPANSJA: fcalls = " << solution::f_calls << ".\n\n";
      if (Sout.good() == true)
        Sout << ps(0) << "\t" << obszar[0] << "\t" << obszar[1] << "\t"
             << solution::f_calls << "\t";
      solution::clear_calls();

      cout << "FIBBONACI:\n";
      opt = fib(ff1T, obszar[0], obszar[1], epsilon, lb,
                ub);               // wywołanie procedury optymalizacji
      cout << opt << endl << endl; // wypisanie wyniku
      if (Sout.good() == true)
        Sout << opt.x(0) << "\t" << opt.y(0) << "\t" << solution::f_calls
             << "\tlokalne\t";
      solution::clear_calls();

      cout << "LAGRANGE:\n";
      opt = lag(ff1T, obszar[0], obszar[1], epsilon, gamma, Nmax, lb,
                ub); // wywołanie procedury optymalizacji
      delete[] obszar;
      cout << opt << endl << endl; // wypisanie wyniku
      if (Sout.good() == true)
  ...
  while (kont == '1') {
    ...
    cout << "\nBEZ EKSPANSJI:\n";
    cout << "FIBBONACI:\n";
    opt = fib(ff1T, -100.0, 100.0, epsilon, lb,
              ub);               // wywołanie procedury optymalizacji
    cout << opt << endl << endl; // wypisanie wyniku
    if (Sout.good() == true)
      Sout << opt.x(0) << "\t" << opt.y(0) << "\t" << solution::f_calls
    ...
    cout << "LAGRANGE:\n";
    opt = lag(ff1T, -100.0, 100.0, epsilon, gamma, Nmax, lb,
              ub);               // wywołanie procedury optymalizacji
    cout << opt << endl << endl; // wypisanie wyniku
    if (Sout.good() == true)
      Sout << opt.x(0) << "\t" << opt.y(0) << "\n"
  ...
  while (kont == '1') {
    ...
    for (int i = 0; i < 1; i++) {
      ...
      cout << "LAGRANGE:\n";
      solution opt_lag =
          lag(ff1R, obszar[0], obszar[1], epsilon, gamma, Nmax, lb, ub);
      cout << opt_lag << endl << endl;

      int fcalls_lagrange = solution::f_calls;

      if (Sout.good() == true) {
        Sout << "Lagrange\t" << opt_lag.x(0) << "\t" << opt_lag.y(0) << "\t"
             << fcalls_lagrange << "\n";
      }
      solution::clear_calls();
      ...
      if (Sout.good() == true) {
        Sout << "Fibonacci\t" << opt_fib.x(0) << "\t" << opt_fib.y(0) << "\t"
             << fcalls_fibonacci << "\n";
      }
      solution::clear_calls();
      ...
      zapiszSymulacje(Y_lag, "symulacja_Lagrange_DA_" +
                                 to_string((int)opt_lag.x(0)) + ".csv");
      ...
      zapiszSymulacje(Y_fib, "symulacja_Fibonacci_DA_" +
                                 to_string((int)opt_fib.x(0)) + ".csv");
  ...
  // // Y0 zawiera warunki pocz�tkowe 	MT = matrix(2, new double[2] { m2d(opt.x),
  //0.5 });	// MT zawiera moment si�y dzia�aj�cy na wahad�o oraz czas
  //dzia�ania matrix* Y = solve_ode(df0, 0, 0.1, 10, Y0, NAN, MT);	//
  // rozwi�zujemy r�wnanie r�niczkowe ofstream Sout("testy_lab1.csv");
  // // definiujemy strumie� do pliku .csv Sout << hcat(Y[0], Y[1]);
  // // zapisyjemy wyniki w pliku Sout.close();
  // // zamykamy strumie� Y[0].~matrix();
  // // usuwamy z pami�ci rozwi�zanie RR Y[1].~matrix();
}
...
void lab2() {
  ...
  srand(time(NULL));
  // Funkcja testowa
  double alfa = 0.0; // wspolczynnik ekspansji (0.0 do 1.0 dla HJ i > 1 dla R -
                     // zmiana w petli, nie tutaj)
  ...
  while (kont == '1') {
    for (int i = 0; i < 1; i++) { // JG:mozna wybrac liczbe powtorzen
      ...
      if (Sout.good() == true)
        Sout << ps(0) << "\t" << ps(1) << '\t';
      opt = HJ(ff2T, ps, krok_s, alfa, epsilon, Nmax, lb,
               ub); // wywołanie procedury optymalizacji
      cout << opt << endl << endl;
      ...
      cout << "ROSENBROCK:\n";
      opt = Rosen(ff2T, ps, matrix(2, 1, krok_s), alfa, beta, epsilon, Nmax, lb,
                  ub); // wywołanie procedury optymalizacji
      cout << opt << endl << endl;
      if (Sout.good() == true) {
        Sout << opt.x(0) << "\t" << opt.x(1) << '\t' << opt.y(0) << "\t"
  ...
  lb = matrix(2, 1, 0.0);
  ub = matrix(2, 1, 20.0), ps = matrix(2, 1, double(rand() % 200001) / 10000.0);
  krok_s = 0.01;
  kont = '1';
  ...
  while (kont == '1') {
    for (int i = 0; i < 1; i++) { // JG:mozna wybrac liczbe powtorzen
      ...
      if (Sout.good() == true)
        Sout << ps(0) << "\t" << ps(1) << '\t';
      opt = HJ(ff2R, ps, krok_s, alfa, epsilon, Nmax, Yref,
               ub); // wywołanie procedury optymalizacji
      cout << opt << endl << endl;
      ...
      cout << "ROSENBROCK:\n";
      opt = Rosen(ff2R, ps, matrix(2, 1, krok_s), alfa, beta, epsilon, Nmax,
                  Yref, ub); // wywołanie procedury optymalizacji
      cout << opt << endl << endl;
      if (Sout.good() == true) {
        Sout << opt.x(0) << "\t" << opt.x(1) << '\t' << opt.y(0) << "\t"
             << solution::f_calls << '\n';
