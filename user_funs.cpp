...
matrix ff0T(matrix x, matrix ud1,
            matrix ud2) // funkcja celu dla przypadku testowego
{
  matrix y; // y zawiera warto�� funkcji celu
  y = pow(x(0) - ud1(0), 2) +
      pow(x(1) - ud1(1), 2); // ud1 zawiera wsp�rz�dne szukanego optimum
  return y;
}

matrix ff0R(matrix x, matrix ud1,
            matrix ud2) // funkcja celu dla problemu rzeczywistego
{
  matrix y;                 // y zawiera warto�� funkcji celu
  matrix Y0 = matrix(2, 1), // Y0 zawiera warunki pocz�tkowe
      MT = matrix(
          2, new double[2]{m2d(x), 0.5}); // MT zawiera moment si�y dzia�aj�cy
                                          // na wahad�o oraz czas dzia�ania
  matrix *Y = solve_ode(df0, 0, 0.1, 10, Y0, ud1,
  ...
  y = abs(
      teta_max -
      m2d(ud1)); // warto�� funkcji celu (ud1 to za�o�one maksymalne wychylenie)
...
matrix df0(double t, matrix Y, matrix ud1, matrix ud2) {
  ...
  dY(1) = ((t <= ud2(1)) * ud2(0) - m * g * l * sin(Y(0)) - b * Y(1)) /
          I; // pochodna z pr�dko�ci to przyspieszenie
  return dY;
}
...
matrix ff1T(matrix x, matrix ud1, matrix ud2) {
  matrix y;
  y = -1.0 * std::cos(0.1 * x(0)) *
          std::exp(-1.0 * std::pow(0.1 * x(0) - 2.0 * M_PI, 2.0)) +
      0.002 * std::pow(0.1 * x(0), 2.0);
  return y;
}
...
matrix ff2T(matrix x, matrix ud1, matrix ud2) {
  matrix y;
  // y = x(0)*x(0) + x(1)*x(1) - std::cos(2.5 * M_PI * x(0)) - std::cos(2.5 *
  // M_PI * x(0)) + 2;
  y = pow(x(0), 2.0) + pow(x(1), 2.0) - cos(2.5 * M_PI * x(0)) -
      cos(2.5 * M_PI * x(1)) + 2;
  return y;
}
...
matrix df2(double t, matrix Y, matrix ud1, matrix ud2) {
  ...
  double I = (one_third * m_r * pow(l, 2)) + (m_c * pow(l, 2));
